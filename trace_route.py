...
def analyze_group_trace_files(file):
...
def parse_icmp(data):
    ...
    embedded_protocol = embedded_ip_header[6]  # Protocol field
    embedded_data = rest_of_icmp[20:]  # Remainder after the IP header

    if embedded_protocol == 1:
        if len(embedded_data) < 8:  # Check if there's enough data for an ICMP header
            return {
                "type": icmp_type,
                "code": icmp_code,
                "checksum": icmp_checksum,
        ...
        embedded_icmp_header = struct.unpack("!BBHHH", embedded_data[:8])
        embedded_icmp_seq = embedded_icmp_header[4]
        return {
            "type": icmp_type,
            "code": icmp_code,
            "checksum": icmp_checksum,
            "identifier": identifier,
            "seq": seq_number,
            "embedded_seq": embedded_icmp_seq,
            "id": embedded_ip_id,
        ...
        if len(embedded_data) < 8:  # Check if there's enough data for a UDP header
            return {
                "type": icmp_type,
                "code": icmp_code,
                "checksum": icmp_checksum,
        ...
        udp_header = struct.unpack("!HHHH", embedded_data[:8])
...
def analyze_traceroute(pcap_file, enable_output):
    ...
    if enable_output:
        ...
        for proto in sorted(protocol_set):
            print(
                f"{proto}: {'ICMP' if proto == 1 else 'UDP' if proto == 17 else 'Other'}"
            )

        if len(fragment_offsets) == 0:
            print(
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        ...
        ttl_rtt = analyze_traceroute(pcap_file, True)

        group1_trace_files = [
            f"PcapTracesAssignment3/group1-trace{i}.pcap" for i in range(1, 6)
        ]
        group2_trace_files = [
